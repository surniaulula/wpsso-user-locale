...
if ( ! class_exists( 'WpssoUlRegister' ) ) {

	class WpssoUlRegister {

		public function __construct() {

			register_activation_hook( WPSSOUL_FILEPATH, array( &$this, 'network_activate' ) );
			//register_deactivation_hook( WPSSOUL_FILEPATH, array( &$this, 'network_deactivate' ) );	// Deactivation method not required.
		...
		private function activate_plugin() {
			$version = WpssoUlConfig::$cf['plugin']['wpssoul']['version'];	// only our config
			if ( class_exists( 'WpssoUtil' ) ) {
				WpssoUtil::save_all_times( 'wpssoul', $version );
			} else {
				WpssoUl::required_notice( true );			// $deactivate = true
			}
		}
